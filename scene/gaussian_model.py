...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int,args):
        ...
        self._semantic_feature = torch.empty(0)  #新增语义特征
    ...
    def segment(self, mask):
        new_gaussians = copy.copy(self)
        ori_num = self.get_xyz.shape[0]
        # 对所有属性进行类型检查和深拷贝/浅拷贝
        for attr_name, attr_value in self.__dict__.items():
            # 检查属性是否为张量，且第一个维度等于 num_points
            # if isinstance(attr_value, torch.Tensor) and attr_value.shape[0] == ori_num:
            #     # 使用布尔掩膜筛选数据，只保留 mask 中为 True 的行
            #     setattr(new_gaussians, attr_name, attr_value[mask].detach().clone().requires_grad_(attr_value.requires_grad))
            if isinstance(attr_value, torch.Tensor) and ori_num in attr_value.shape:
                # 找到第一个等于 ori_num 的维度索引
                dim_to_filter = attr_value.shape.index(ori_num)
                # 使用布尔掩膜筛选数据
                filtered_tensor = attr_value.index_select(dim_to_filter, torch.nonzero(mask, as_tuple=True)[0])
                # 设置新的张量
                setattr(new_gaussians, attr_name, filtered_tensor.detach().clone().requires_grad_(attr_value.requires_grad))
            elif isinstance(attr_value, torch.Tensor) and attr_value.shape[0] != ori_num:
                setattr(new_gaussians, attr_name, attr_value.detach().clone().requires_grad_(attr_value.requires_grad))
            else:
                # 对非 Tensor 的其他类型做深拷贝，确保嵌套结构也被复制
                setattr(new_gaussians, attr_name, copy.deepcopy(attr_value))
        #对clone的物体分割mask掩膜分割
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float, semantic_feature_size : int, speedup: bool):
        ...
        self._semantic_feature = torch.zeros(fused_point_cloud.shape[0], semantic_feature_size, 1).float().cuda() #初始化语义特征
    ...
    def training_setup(self, training_args):
        ...
        self.prune_params_names=["xyz","f_intr","opacity","scaling","rotation","semantic_feature"]
    ...
    def save_ckpt_ply(self, path):
        ...
        semantic_feature = self._semantic_feature.detach().transpose(1, 2).flatten(         #保存语义特征预处理
        ...
        attributes = np.concatenate((xyz, normals, f_intr, opacities, scale, rotation, semantic_feature), axis=1)
    ...
    def forward(self,viewpoint_camera,store_cache=False):
        ...
        if self.use_okmap:
            ...
            _point_features,self.map_pts_norm=sample_from_feature_maps(feature_maps[:self.map_num,...],_xyz,self.box_coord[:,:self.map_num,:],self.feature_maps_combine)
            ...
            if self.use_kmap_pjmap:
                ...
                if self.use_xw_init_box_coord:
                    ...
                    _point_features1,project_mask=project2d(_xyz,viewpoint_camera.world_view_transform,viewpoint_camera.intrinsic_matrix,box_coord1,feature_maps[-1,...].unsqueeze(0))
                else: raise NotImplementedError
                    # _point_features0,self.map_pts_norm=sample_from_feature_maps(feature_maps[:self.map_num-1,...],_xyz,box_coord2,self.coord_scale,self.feature_maps_combine)
                    # _point_features1,project_mask=project2d(_xyz,viewpoint_camera.world_view_transform,viewpoint_camera.intrinsic_mmatrix,box_coord1,feature_maps[-1,...].unsqueeze(0))
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_tensor["semantic_feature"] = self._semantic_feature[selected_pts_mask].repeat(N,1,1)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        new_tensor["semantic_feature"] = self._semantic_feature[selected_pts_mask]
