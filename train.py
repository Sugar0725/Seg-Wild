...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, debug_from,args):
    ...
    for iteration in range(first_iter, opt.iterations + 1):    
        ...
        #Add semantic feature loss
        if args.use_semantic_feature and iteration>args.semantic_feature_iters: #5000
            gt_feature_map = viewpoint_cam.semantic_feature.cuda()
            feature_map = F.interpolate(feature_map.unsqueeze(0),size=(gt_feature_map.shape[1], gt_feature_map.shape[2]), mode='bilinear',align_corners=True).squeeze(0)
            if dataset.speedup:
                feature_map = cnn_decoder(feature_map)
            #变换形状使瞬态mask对齐feature_map
            feature_map_mask = gaussians.features_mask
            feature_map_mask = torch.nn.functional.interpolate(feature_map_mask,size=(gt_feature_map.shape[-2:]))
            Ll1_feature = l1_loss(feature_map * feature_map_mask, gt_feature_map * feature_map_mask)
            loss += Ll1_feature * args.semantic_feature_loss_coef
            if args.use_sam_masks:
                #correspendence Loss 使相同特征之间更加紧凑
                # SAGA论文中的公式（8）
                sam_masks = torch.load(os.path.join(args.source_path, "sam_masks", f"{viewpoint_cam.uid:05d}.pt"))
                if sam_masks.shape[-2:] != feature_map.shape[-2:]:
                    sam_masks = F.interpolate(sam_masks.unsqueeze(0), size=feature_map.shape[-2:], mode='nearest').squeeze(0)
                NHW = sam_masks.to("cuda")
                N, H, W = NHW.shape
                NL = NHW.view(N, -1)
                ...
                # 论文中的公式（9）
                norm_rendered_feature = torch.nn.functional.normalize(feature_map, dim=0, p=2)
                correspondence = torch.relu(torch.einsum('CHW,CJK->HWJK', norm_rendered_feature, norm_rendered_feature))

                # 论文中的公式（10）求correspendence Loss
                corr_loss = -HWHW * correspondence

                loss += corr_loss.mean() * 0.3

        elif args.use_semantic_feature and iteration<args.semantic_feature_iters: #如果小于semantic_feature_iters轮数不参与训练
            Ll1_feature = 0
            loss += Ll1_feature * args.semantic_feature_loss_coef
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_sam_masks", action='store_true', default=False)
