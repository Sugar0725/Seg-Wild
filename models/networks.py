...
def sf_linear_transformation(input_tensor, output_dim=512):
    seed = 42
    torch.manual_seed(seed)
    transformation_matrix = torch.randn(input_tensor.shape[0], input_tensor.shape[-1], output_dim).requires_grad_(
        False).cuda()
...
def fmap_linear_transformation(input_tensor, output_dim):
    ...
    transformed_tensor = torch.matmul(transformation_matrix.T,
                                      reshaped_tensor)  # (512, 16) (16, 360*480) -> (512, 360*480)
