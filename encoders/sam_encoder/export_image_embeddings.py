...
#命令行：--checkpoint D:\Pycharm\Pycharm_Project\SegAnyGAussians-main\dependencies\sam_ckpt\sam_vit_h_4b8939.pth --model-type vit_h --input H:\PycharmProject\Gaussian-Wild-main-feature\data/brandenburg_gate/dense/images --output H:\PycharmProject\Gaussian-Wild-main-feature\data/brandenburg_gate/sam_embeddings
...
def main(args: argparse.Namespace) -> None:
    ...
    for t in targets:
        ...
        # 判断长宽比并调整裁剪逻辑
        if img_h >= img_w:
            # 高度更长，将高度固定为 fea_h 像素，宽度按比例缩放
            new_h = fea_h
            scale_factor = fea_h / img_h
            new_w = int(img_w * scale_factor + 0.5)

            # 计算裁剪后的特征图宽度
            cropped_h = new_h
            cropped_w = new_w
            image_embedding_tensor_cropped = image_embedding_tensor[:, :cropped_h, :cropped_w]
        else:
            # 宽度更长或相等，按照宽高比例裁剪高度
            cropped_h = int(fea_w / img_w * img_h + 0.5)
            image_embedding_tensor_cropped = image_embedding_tensor[:, :cropped_h, :]
